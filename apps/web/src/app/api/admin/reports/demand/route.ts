...
import { createDb, validateSession, requirePermission, getMissedDemandReport } from '@pavilion/db';
...
export async function GET(request: Request) {
  try {
    ...
    const data = await getMissedDemandReport(db, fromDate, toDate);
    ...
    return NextResponse.json({ ok: false, error: 'Failed to generate demand report' }, { status: 500 });
  }
}
